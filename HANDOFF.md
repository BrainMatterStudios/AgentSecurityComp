...
## 0. TL;DR
- **Competition:** Kaggle "AI Agent Security: Multi-Step Tool Attacks" ($50k, prize is the **private** LB). **Entry deadline 2026-08-25** (read off the official page by a clean-room agent 2026-08-02) — still **VERIFY the FINAL-SUBMISSION deadline + selection count** (Kaggle entry ≠ final; the selection count decides pure-vs-blended hedge structure).
...
## 10. Running state / open items at handoff (2026-08-02)
- **L14 (5 subs) PENDING** — fired 2026-08-02 09:09 UTC, scoring ~3h (L13 took ~3h). Poller `dev/_poll_l14.py` **NOT launched** (classifier-blocked). Next session: `python3 -m kaggle competitions submissions ai-agent-security-multi-step-tool-attacks` or `! caffeinate -i python3 dev/_poll_l14.py`. Refs/versions in §4b + `logs/l14_versions.json`. Today's quota is SPENT (5/5).
