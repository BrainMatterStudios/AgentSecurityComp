...
- **ACTION (top priority): fix the deputy hedge.** Needs (a) a terminating deputy template (force one-line + "reply OK only, no other tool calls", mirroring exfil's close_ok/terse_ok) so replay cost is low+consistent, (b) a much lower FRAC (~80-85) as void insurance, (c) re-test on Kaggle. Until it scores non-void, we have NO selectable private hedge.
