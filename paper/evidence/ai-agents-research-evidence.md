...
### Independent early live-row recheck

The working-note artifact includes L7 and L9 but omits L6. The rows below
preserve the separate Task 8 Kaggle API recheck completed at
2026-08-16T11:26:07Z. A visible score on a row whose API status is `ERROR` is
not a completed live effect estimate. No returned description matched the
planned L8 campaign; the later returned packing descriptions are labeled L9.

| Campaign | Ref | API status | Visible score | Returned configuration |
| --- | ---: | --- | ---: | --- |
| L6 | 54986870 | `ERROR` | 64.800 | Control, GPT index 13 at 840 plus Gemma 600 |
| L6 | 54986885 | `ERROR` | 80.145 | Adaptive replay-safe, fraction 0.97 |
| L6 | 54986896 | `ERROR` | 76.725 | Adaptive replay-safe, fraction 0.90 |
| L6 | 54986905 | `ERROR` | 68.445 | Adaptive replay-safe, fraction 0.82 |
| L6 | 54986916 | `ERROR` | none | Static hedge, GPT 840 plus Gemma 620 |
| L7 | 55013491 | `ERROR` | 85.770 | Fraction-99 control without `close_ok` |
| L7 | 55013500 | `ERROR` | 87.210 | Fraction-99 `close_ok` arm |
| L7 | 55013507 | `ERROR` | 87.480 | Fraction-97 `close_ok` arm; no same-fraction no-close control |
| L9 | 55040336 | `ERROR` | 89.640 | Single-post control, fraction 0.97 |
| L9 | 55040351 | `ERROR` | 80.015 | Packed K=2, fraction 0.97 |
| L9 | 55040363 | `ERROR` | 75.945 | Packed K=4, fraction 0.97 |
| L9 | 55040369 | `ERROR` | 73.665 | Packed K=8, fraction 0.97 |
| L9 | 55040377 | `ERROR` | 70.645 | Packed K=16, fraction 0.95 |
...
| ASC | 2026-07-25 | Adapt output size to replay-safe observed limits. | external | Mechanism attributed to higher-scoring public solutions. | High. | Approved a bounded ladder. | Replay-safe adaptive sizing passed local checks. | All five identified L6 rows were `ERROR`; the adaptive row showed 80.145 versus a visible 64.800 control, which is not a completed effect estimate. | partially supported | A locally verified external mechanism remains externally unestablished when every identified live row errors. | AS-S04 |
| ASC | 2026-07-26 | Stop decoding after `close_ok` to reduce per-candidate latency. | agent | Local timing of 1.03 s versus 1.23 s. | Called it a decisive roughly 20% lever. | Submitted a controlled ladder. | Clear local latency reduction. | Matched L7 rows visibly differed by 1.440 points, but both were `ERROR`; no completed live effect was established. | partially supported | A visible difference on error rows cannot calibrate external transfer. | AS-S05 |
...
| ARC | 2026-06-29 | Double-reset between plays to improve geodesic efficiency. | agent | Local state-machine analysis. | High. | Challenged the result against competition mode. | A handoff recorded 7–109×; a later eval-faithful artifact reported 2.6–152× on 15/25 games after the no-op repair. | Second play returned HTTP 400. | invalid experiment | Preserve differing local measurements and test lifecycle semantics in the actual evaluation mode. | ARC-S03 |
...
| ARC | 2026-07-11 | Serve a fine-tuned adapter that appeared to beat base. | mixed | Local 1.26 result and human interest in fine-tuning. | High before serving audit. | Asked for proof of the deployed model identity. | The apparent gain was generated by base, not the adapter. | The adapter never served. | invalid experiment | Serving identity is part of experimental validity. | ARC-S05 |
| ARC | 2026-08-01 | Measure A/A evaluation noise before reading small deltas. | mixed | Replicate variability and human skepticism. | High. | Required registered gates. | RMS per game was 0.707 levels; two registered gates were under-powered. | No live-effect claim was made; the protocol amendment preserved the result. | confirmed | Calibrate the instrument before interpreting changes. | ARC-S06 |
...
| AS-S03 | AgentSecurityComp commit `1ac76a02e7d1de542550d4f8ea3dc5650d5957a2`; `1ac76a0:attack.py#L381-L386,#L656,#L687,#L697` (marker payload and call sites); `1ac76a0:dev/validate_312_score.py#L1-L5,#L24-L43,#L60-L71` (end-to-end marker versus old-token validation) |
| AS-S04 | AgentSecurityComp commit `e85a7ba`; `e85a7ba:dev/_replay_safe_smoke.py#L1-L6,#L29-L60` (local replay-safety checks); Task 8 independent live-row recheck, refs `54986870`, `54986885`, `54986896`, `54986905`, and `54986916` (all `ERROR`) |
| AS-S05 | AgentSecurityComp commit `242a855`; `claude:AgentSecurityComp/25c84940-5ede-4850-a639-5579fdef6ebe.jsonl#L255-L258,#L491`, continuation linked to canonical `f1ef3669-1d5f-4caf-ab9b-c4eedf643569`; Task 8 independent live-row recheck, refs `55013491`, `55013500`, and `55013507` (all `ERROR`) |
| AS-S06 | AgentSecurityComp commit `77862ef`; `claude:AgentSecurityComp/25c84940-5ede-4850-a639-5579fdef6ebe.jsonl#L566`, continuation linked to canonical `f1ef3669-1d5f-4caf-ab9b-c4eedf643569`; `77862ef:dev/_build_l8.py#L43-L66` (planned L8 descriptions); Task 8 independent live-row recheck found no returned L8 description and found L9 refs `55040336`, `55040351`, `55040363`, `55040369`, and `55040377` all `ERROR` |
...
| OC-S04 | OpenCode parent `ses_019d88ca1ffe3iOYWxPRUUcfAA`: scope `prt_fe62773a10020CStB4KUZR7l0U` (2026-08-09 10:52:53Z), synthesis `prt_fe74f3faf001pnW7paA43QMreY` (16:15:58Z); children `ses_019ca316cffegBxl0NpwvitlLO`, `ses_019c9dd5bffeuX588N88Xn5WSF`, `ses_019c9b9e2ffexGiL2Ode8mpfKV`, `ses_01990f651ffeMalEJ9WV0K773P`, `ses_017f9fdb1ffeBuyqRX0He6wj71`, `ses_017f9dc39ffeIbLmAP4q7R6DLU`, `ses_017f9b1a7ffeeTpM2a9aZHaH0w`, `ses_017f98cb8ffeu7uREep8fiwApw`, `ses_017f96940ffeOZMamaLyLZjIEo`; later corpus refutation at cutoff-ancestor commit `4a17e8f:docs/RESULTS-2026-08-08-overnight-gates.md#L143-L149` |
| OC-S05 | OpenCode parent `ses_012d743fcffeBoq8D4McmfGQTH`: human challenge `prt_fed28bc42002PXmQG8QKoFfAbb` (2026-08-10 19:31:37Z), plan `prt_fed3e0b84001O1GyQL97xzc4a6` (19:54:54Z); later Claude repair at cutoff-ancestor commit `fbd1197`, with exact no-op diagnosis and namespace fix in `ebe5b3e:submission/_duck_mem/harness_mem.py#L26-L30,#L145-L164`; live result at `ebe5b3e:docs/submission-ledger.json#L35-L42`, submission `55488796`, score 0.83 within the registered 0.69-1.30 band; no child sessions |
...
This inventory was checked against the full primary paper where accessible and
against the publisher or conference record for bibliographic status. The cutoff
for mutable policy pages is 2026-08-16. Findings below are paraphrases, not
quotations. A benchmark task, a manuscript-generation pipeline, or a bounded
laboratory loop is not evidence of general scientific autonomy.

### Verified reference inventory

`et al.` is used only where the publisher record has a long author list; the
linked record was checked for the full list.

| ID | Verified bibliographic identity | Status and stable identifier | Opened primary or official source |
| --- | --- | --- | --- |
| L01 | Ross D. King, Kenneth E. Whelan, Ffion M. Jones, Philip G. K. Reiser, Christopher H. Bryant, Stephen H. Muggleton, Douglas B. Kell and Stephen G. Oliver, “Functional genomic hypothesis generation and experimentation by a robot scientist” (2004), *Nature* 427, 247–252 | Peer-reviewed article; DOI `10.1038/nature02236` | [Nature version of record](https://www.nature.com/articles/nature02236) |
| L02 | Benjamin Burger, Phillip M. Maffettone, Vladimir V. Gusev et al., “A mobile robotic chemist” (2020), *Nature* 583, 237–241 | Peer-reviewed article; DOI `10.1038/s41586-020-2442-2` | [Nature version of record](https://www.nature.com/articles/s41586-020-2442-2) |
| L03 | Nathan J. Szymanski, Bernardus Rendy, Yuxing Fei et al., “An autonomous laboratory for the accelerated synthesis of inorganic materials” (2023), *Nature* 624, 86–91 | Peer-reviewed article; DOI `10.1038/s41586-023-06734-w` | [Nature version of record](https://www.nature.com/articles/s41586-023-06734-w) |
| L04 | Yang Jeong Park, Daniel Kaplan, Zhichu Ren, Chia-Wei Hsu, Changhao Li, Haowei Xu, Sipei Li and Ju Li, “Can ChatGPT be used to generate scientific hypotheses?” (online 2023; issue 2024), *Journal of Materiomics* 10(3), 578–584 | Peer-reviewed perspective; DOI `10.1016/j.jmat.2023.08.007` | [Elsevier version of record](https://www.sciencedirect.com/science/article/pii/S2352847823001557) |
| L05 | William H. Walters and Esther Isabelle Wilder, “Fabrication and errors in the bibliographic citations generated by ChatGPT” (2023), *Scientific Reports* 13, 14045 | Peer-reviewed article; DOI `10.1038/s41598-023-41032-5` | [Scientific Reports version of record](https://www.nature.com/articles/s41598-023-41032-5) |
| L06 | Ziru Chen, Shijie Chen, Yuting Ning et al., “ScienceAgentBench: Toward Rigorous Assessment of Language Agents for Data-Driven Scientific Discovery” (2025), ICLR 2025 | Peer-reviewed conference paper; OpenReview `6z4YKr0GK6`; arXiv `2410.05080` | [Official ICLR paper record](https://openreview.net/forum?id=6z4YKr0GK6) and [authors' benchmark site](https://osu-nlp-group.github.io/ScienceAgentBench/) |
| L07 | Juraj Gottweis, Wei-Hung Weng, Alexander Daryin et al., “Accelerating scientific discovery with Co-Scientist” (2026), *Nature* 655, 487–496 | Peer-reviewed article; DOI `10.1038/s41586-026-10644-y` | [Nature version of record](https://www.nature.com/articles/s41586-026-10644-y) |
| L08 | Tal Ifargan, Lukas Hafner, Maor Kern, Ori Alcalay and Roy Kishony, “Autonomous LLM-driven research from data to human-verifiable research papers” (2024) | Preprint; arXiv `2404.17605`, version 1 | [arXiv primary manuscript](https://arxiv.org/abs/2404.17605) |
| L09 | Chris Lu, Cong Lu, Robert Tjarko Lange, Jakob Foerster, Jeff Clune and David Ha, “The AI Scientist: Towards Fully Automated Open-Ended Scientific Discovery” (2024) | Preprint; arXiv `2408.06292`, version 3 | [arXiv full text](https://arxiv.org/html/2408.06292) |
| L10 | Bodhisattwa Prasad Majumder, Harshit Surana, Dhruv Agarwal, Bhavana Dalvi Mishra, Abhijeetsingh Meena, Aryan Prakhar, Tirth Vora, Tushar Khot, Ashish Sabharwal and Peter Clark, “DiscoveryBench: Towards Data-Driven Discovery with Large Language Models” (2024) | Preprint; arXiv `2407.01725`, version 1 | [arXiv full text](https://arxiv.org/html/2407.01725) |
| L11 | Zachary S. Siegel, Sayash Kapoor, Nitya Nadgir, Benedikt Stroebl and Arvind Narayanan, “CORE-Bench: Fostering the Credibility of Published Research Through a Computational Reproducibility Agent Benchmark” (2024; revised 2026) | Preprint; arXiv `2409.11363`, version 2 | [arXiv full text](https://arxiv.org/html/2409.11363) |
| L12 | Michael D. Skarlinski, Sam Cox, Jon M. Laurent, James D. Braza, Michaela Hinks, Michael J. Hammerling, Manvitha Ponnapati, Samuel G. Rodriques and Andrew D. White, “Language agents achieve superhuman synthesis of scientific knowledge” (2024) | Preprint; arXiv `2409.13740`, version 2 | [arXiv primary manuscript](https://arxiv.org/abs/2409.13740) |
| L13 | Chenglei Si, Diyi Yang and Tatsunori Hashimoto, “Can LLMs Generate Novel Research Ideas? A Large-Scale Human Study with 100+ NLP Researchers” (2024) | Preprint; arXiv `2409.04109`, version 1 | [arXiv primary manuscript](https://arxiv.org/abs/2409.04109) |
| L14 | Mike D'Arcy, Tom Hope, Larry Birnbaum and Doug Downey, “MARG: Multi-Agent Review Generation for Scientific Papers” (2024) | Preprint; arXiv `2401.04259`, version 1 | [arXiv primary manuscript](https://arxiv.org/abs/2401.04259) |
| L15 | Giulio Starace, Oliver Jaffe, Dane Sherburn et al., “PaperBench: Evaluating AI's Ability to Replicate AI Research” (2025) | Official benchmark release and accompanying preprint; arXiv `2504.01848` | [OpenAI release](https://openai.com/index/paperbench/) and [primary paper](https://arxiv.org/abs/2504.01848) |
| P01 | International Committee of Medical Journal Editors, “Use of AI by Authors” | Current editorial recommendation; mutable page checked 2026-08-16 | [ICMJE guidance](https://www.icmje.org/recommendations/browse/artificial-intelligence/ai-use-by-authors.html) |
| P02 | Springer Nature, “AI guidance for researchers and communities” | Current publisher policy; mutable page checked 2026-08-16 | [Springer Nature guidance](https://group.springernature.com/gp/group/ai/ai-guidance-for-our-researchers-and-communities) |

### Claim-level source matrix

| Source | Status | Scientific task | Supported finding | Limitation | Manuscript claim |
| --- | --- | --- | --- | --- | --- |
| L01, King et al. | Peer-reviewed article | Generate, select and experimentally test gene-function hypotheses in a yeast metabolic model | The implemented system closed a hypothesis–experiment–interpretation loop, and its experiment-selection strategy was competitive with the human comparator in this defined setting. | The task used deletion mutants, auxotrophic growth assays and a hand-built logical model of one pathway; it does not establish domain-general discovery. | Early “robot scientist” work demonstrated bounded closed-loop experimentation, not general autonomy. |
| L02, Burger et al. | Peer-reviewed article | Optimize photocatalyst mixtures with a mobile robot and Bayesian search | Across a fixed ten-variable search, the robot executed 688 experiments over eight days and found mixtures more active than the initial formulations. | Human researchers conceived the study and set the five hypotheses; the chemistry, objective, apparatus and search space were predefined. | Laboratory autonomy can be substantial inside a human-specified experimental envelope. |
| L03, Szymanski et al. | Peer-reviewed article | Plan, execute and interpret solid-state inorganic synthesis | A-Lab integrated literature-trained models, computation, robotics and active learning and realized 36 of 57 selected targets during 17 days of operation. | Humans selected the air-stable targets and allowable precursors; some X-ray diffraction calls were inconclusive, and manual follow-up exposed failure modes outside the active-learning policy. | Closed-loop materials platforms can accelerate a narrow synthesis program while retaining important human and instrumentation boundaries. |
| L04, Park et al. | Peer-reviewed perspective | Generate materials-science hypotheses with GPT-4 prompts | The authors present examples suggesting that an LLM can organize background knowledge into potentially testable hypotheses, while explicitly reporting a high error rate and a need for expert evaluation. | This is a perspective and demonstration, not a controlled cross-domain trial or evidence that the hypotheses produced successful discoveries. | LLM ideation is plausibly useful for hypothesis formation but requires expert screening and empirical validation. |
| L05, Walters and Wilder | Peer-reviewed article | Generate literature reviews and bibliographic citations | In 84 generated reviews across 42 topics, both tested ChatGPT versions produced fabricated citations and errors in real citations, although GPT-4 performed better than GPT-3.5. | The prompts and model snapshots were from April 2023 and emphasized short general-topic student reviews, so the rates are not current universal error rates. | Agent-produced literature and references require source-by-source verification; apparent citation form is not provenance. |
| L06, Chen et al. | Peer-reviewed ICLR 2025 paper | Write executable programs for individual data-driven scientific workflow tasks | ScienceAgentBench turns 102 tasks from 44 papers and four disciplines into verifiable coding tasks; the reported baselines solved only a minority even with three attempts and optional expert knowledge. | Outputs are self-contained Python programs for selected workflow steps, not end-to-end projects, laboratory work or open-ended discovery. | Task-level benchmark results argue against inferring broad research autonomy from isolated scientific coding successes. |
...
### Source-opening and claim controls

- Every URL in the inventory above was opened on 2026-08-16. Sixteen returned
  HTTP 200 directly in the command-line resolution check. Elsevier and OpenAI
  returned HTTP 403 to that automated client, but their official pages were
  opened through the browser index and exposed the version-of-record and
  release content. OpenReview returned HTTP 200 after redirecting to an anti-
  bot interstitial; its official PDF/search record and the authors' linked
  benchmark site independently exposed the ICLR 2025 identity and contents.
- Full text, results, discussion or limitations were checked for every research
  row; no finding here was admitted solely from a title match. Where arXiv did
  not expose HTML, the primary PDF was opened and inspected.
- Status labels describe the cited object: an arXiv posting stays labelled
  `preprint` here even if a related version later appears elsewhere. PaperBench
  is labelled an official benchmark release rather than peer-reviewed work.
- No statistic is generalized beyond its source's models, task set, metric and
  evaluation conditions. In particular, automated-review scores, human-study
  judgments and benchmark percentages are not measures of general scientific
  autonomy or real-world productivity.
- The private episode and quote ledgers above remain the authority for the two
  cases. Quote IDs are thematic rather than chronological: Q04 (2026-06-30)
  predates Q02 (2026-07-01), so Q04 is not a retraction of Q02.
...
- The Stage 2b verdict used by ARC-S11 is an audit-time mutable artifact outside
  the ARC Git cutoff. Its SHA-256, file mtime, and audit extraction timestamp are
