#!/usr/bin/env python3
"""Poll L20 fast-emit results until they score. Matches on 'L20' in description. Logs to logs/l20_results.log."""
...
LOG = os.path.join(ROOT, "logs", "l20_results.log")
...
def main():
    log("polling L20 (fast-emit) results...")
    for _ in range(300):
        rows = poll()
        ...
        time.sleep(90)
    rows = poll()
    log("=== L20 RESULTS (fast-emit target >118.0) ===")
    for r, (v, line) in sorted(rows.items()):
        lbl = re.search(r"(L20 [^S]+?)\s{2,}", line)
